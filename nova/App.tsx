...
import { User, Media, Genre, AdType } from './types';
...
const Home: React.FC<{ onPlay: (m: Media) => void }> = ({ onPlay }) => {
  ...
  // const featured = MOCK_MEDIA.find(m => m.isFeatured) || MOCK_MEDIA[0];
  const [media, setMedia] = useState([]);
  const [adBanner, setAdBanner] = useState<any | null>(null);
  // const adBanner = useMemo(() => adEngine.getAdForPlacement(AdType.BANNER), []);

  // useEffect(() => {
  //   getPersonalizedRecommendations(MOCK_USER.watchHistory).then(setRecs);
  // }, []);
  useEffect(() => {
  // Load AI recommendations (existing logic)
  getPersonalizedRecommendations(MOCK_USER.watchHistory).then(setRecs);

  // Fetch ads from Flask backend
  async function loadAds() {
    try {
      const ads = await fetchAds();

      // Pick first ad as banner for now
      if (ads.length > 0) {
        setAdBanner(ads[0]);
      }
    } catch (err) {
  ...
  return (
    <div className="p-8 space-y-12">
      ...
      {/* Rows */}
      <MediaRow title="Trending Throwbacks" items={MOCK_MEDIA} onPlay={onPlay} />
      
      {/* Dynamic Recommendation Row */}
      {recs.length > 0 && (
        <MediaRow title={`Because you watched ${MOCK_MEDIA.find(m => m.id === MOCK_USER.watchHistory[0])?.title}`} items={recs} onPlay={onPlay} />
      )}

      {/* Ad Banner */}
      {/* {adBanner && (
        <section className="py-8">
           <a href={adBanner.linkUrl} target="_blank" rel="noreferrer" className="block relative h-24 rounded-xl overflow-hidden border border-indigo-500/20 shadow-lg shadow-indigo-500/5 group">
             <img src={adBanner.imageUrl} className="w-full h-full object-cover opacity-60 group-hover:scale-105 transition-transform duration-500" alt="Ad" />
             <div className="absolute inset-0 flex items-center justify-between px-12 bg-indigo-900/10">
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-1">Sponsored</span>
                  <span className="text-xl font-display font-bold text-white">{adBanner.name}</span>
                </div>
                <div className="px-6 py-2 bg-white text-black font-bold rounded-lg text-sm group-hover:bg-indigo-400 group-hover:text-white transition-colors">Shop Now</div>
             </div>
           </a>
        </section>
      )} */}
  ...
  <section className="py-8">
    <div className="block relative h-24 rounded-xl overflow-hidden border border-indigo-500/20 shadow-lg shadow-indigo-500/5 group">
      ...
      {/* Using backend media_url */}
      <video
        src={adBanner.media_url}
      ...
      <div className="absolute inset-0 flex items-center justify-between px-12 bg-indigo-900/20">
        ...
        <div className="px-6 py-2 bg-white text-black font-bold rounded-lg text-sm">
          Learn More
        </div>
      </div>
    </div>
      ...
      <MediaRow title="Original Sitcoms" items={MOCK_MEDIA.filter(m => m.genre === Genre.SITCOM)} onPlay={onPlay} />
      <MediaRow title="Sci-Fi Anthology" items={MOCK_MEDIA.filter(m => m.genre === Genre.SCI_FI)} onPlay={onPlay} />
    </div>
  );
};
